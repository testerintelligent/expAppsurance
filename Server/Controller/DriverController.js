...
exports.createDriver = async (req, res) => {
  try {
    ...
    if (!submission) {
      return res.status(404).json({ message: `Submission not found for ID: ${submissionId}` });
    }
...
exports.getDriversBySubmission = async (req, res) => {
  try {
    ...
    if (!drivers || drivers.length === 0) {
      return res.status(404).json({ message: `No drivers found for submission ${submissionId}` });
    }
...
exports.updateDriver = async (req, res) => {
  try {
    ...
    const driver = await Driver.findByIdAndUpdate(driverId, req.body, { new: true });
    if (!driver) {
      return res.status(404).json({ message: `Driver not found with ID: ${driverId}` });
    }
