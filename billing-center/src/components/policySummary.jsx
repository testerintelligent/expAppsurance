  ...
  return (
    <div style={styles.container}>
      <div style={{ marginBottom: 12 }}>
        ...
        <button
          onClick={() =>
            alert("Download PDF - implement server-side PDF generation")
          }
        >
          Download PDF
        </button>
        ...
        <div>
          <div style={styles.section}>
            ...
            <div style={styles.row}>
              <div style={styles.label}>Effective</div>
              <div style={styles.value}>{formatDate(policy.effectiveDate)}</div>
            </div>
            <div style={styles.row}>
              <div style={styles.label}>Expiry</div>
              <div style={styles.value}>{formatDate(policy.expiryDate)}</div>
            </div>
            <div style={styles.row}>
              <div style={styles.label}>Payment Schedule</div>
              <div style={styles.value}>{policy.paymentSchedule || "-"}</div>
            </div>
          </div>
          ...
          <div style={styles.section}>
            ...
            <div style={styles.row}>
              <div style={styles.label}>Total Premium</div>
              <div style={styles.money}>
                {formatCurrency(policy.totalPremium)}
              </div>
            </div>
            <div style={styles.row}>
              <div style={styles.label}>Taxes</div>
              <div style={styles.money}>{formatCurrency(policy.taxes)}</div>
            </div>
            <div style={{ height: 8 }} />
            <div style={{ ...styles.row, fontSize: 16 }}>
              <div style={styles.label}>Total</div>
              <div style={{ ...styles.value, ...styles.emphasizedMoney }}>
                {formatCurrency(policy.totalCost)}
              </div>
            </div>
          </div>
          ...
          <div style={styles.section}>
            ...
            <div style={styles.row}>
              ...
              <div style={styles.value}>{policy.billingMethod || "-"}</div>
            </div>
            <div style={styles.row}>
              <div style={styles.label}>Payment Ref</div>
              <div style={styles.value}>{policy.paymentRef || "-"}</div>
            </div>
        ...
        <aside>
          ...
          >
            ...
            <div style={{ fontWeight: 600 }}>
              {policy.accountId?.accountHolderName ||
                policy.accountHolderName ||
                "N/A"}
            </div>
            <div style={{ color: "#6b7280", marginTop: 8 }}>Account</div>
            <div style={{ fontWeight: 600 }}>
              {policy.accountId?.accountId || policy.accountNumber || "N/A"}
            </div>

            <div style={{ color: "#6b7280", marginTop: 8 }}>Contact</div>
            <div style={{ fontWeight: 600 }}>
              {policy.contactId?.email || policy.contactId?.phone || "N/A"}
            </div>
          </div>
